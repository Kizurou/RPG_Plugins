...
#include "AbilitySimpleFailureMessage.h"
...
bool UBaseGameplayAbility::ChangeActivationGroup(EAbilityActivationGroup NewGroup)
{
	...
	if (ActivationGroup != NewGroup)
	{
		...
		BaseASC->RemoveAbilityFromActivationGroup(ActivationGroup, this);
		BaseASC->AddAbilityToActivationGroup(NewGroup, this);
...
bool UBaseGameplayAbility::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	UBaseAbilitySystemComponent* BaseASC = CastChecked<UBaseAbilitySystemComponent>(ActorInfo->AbilitySystemComponent.Get());
	...
	if (BaseASC->IsActivationGroupBlocked(ActivationGroup))
	{
...
void UBaseGameplayAbility::SetCanBeCanceled(bool bCanBeCanceled)
{
	...
	{
		UE_LOG(LogBaseAbilitySystem, Error, TEXT("SetCanBeCanceled: Ability [%s] can not block canceling because its activation group is replaceable."), *GetName());
		return;
	}
...
bool UBaseGameplayAbility::CheckCost(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, OUT FGameplayTagContainer* OptionalRelevantTags /*= nullptr*/) const
{
...
void UBaseGameplayAbility::ApplyCost(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo) const
{
	...
	auto DetermineIfAbilityHitTarget = [&]()
	{
		...
		{
			if (UBaseAbilitySystemComponent* ASC = Cast<UBaseAbilitySystemComponent>(ActorInfo->AbilitySystemComponent.Get()))
			{
...
FGameplayEffectContextHandle UBaseGameplayAbility::MakeEffectContext(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo) const
{
	...
	const IBaseAbilitySourceInterface* AbilitySource = nullptr;
...
void UBaseGameplayAbility::ApplyAbilityTagsToGameplayEffectSpec(FGameplayEffectSpec& Spec, FGameplayAbilitySpec* AbilitySpec) const
{
	...
	{
		// @TODO Check use cases of tags on physics mats and investigate if it makes sense to live within abilities framework
...
bool UBaseGameplayAbility::DoesAbilitySatisfyTagRequirements(const UAbilitySystemComponent& AbilitySystemComponent, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, OUT FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (BaseASC)
	{
		BaseASC->GetAdditionalActivationTagRequirements(AbilityTags, AllRequiredTags, AllBlockedTags);
	}
...
void UBaseGameplayAbility::GetAbilitySource(FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, float& OutSourceLevel, const IBaseAbilitySourceInterface*& OutAbilitySource, AActor*& OutEffectCauser) const
{
	OutSourceLevel = 0.0f;
	...
	OutAbilitySource = Cast<IBaseAbilitySourceInterface>(SourceObject);
}
