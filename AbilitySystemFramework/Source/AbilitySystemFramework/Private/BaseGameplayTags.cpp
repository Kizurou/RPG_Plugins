...
void FBaseGameplayTags::AddCustomMovementModeTag(FGameplayTag& OutTag, const ANSICHAR* TagName, uint8 CustomMovementMode)
{
...
FGameplayTag FBaseGameplayTags::FindTagByString(FString TagString, bool bMatchPartialString)
{
	...
	if (!Tag.IsValid() && bMatchPartialString)
	{
		...
		for (const FGameplayTag TestTag : AllTags)
		{
			...
			{
				UE_LOG(LogBaseAbilitySystem, Display, TEXT("Could not find exact match for tag [%s] but found partial match on tag [%s]."), *TagString, *TestTag.ToString());
				Tag = TestTag;
				break;
			}
		}
